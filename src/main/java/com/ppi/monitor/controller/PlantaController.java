...
public class PlantaController {
    ...
    @Autowired
    private IPlantaBusiness plantaBusiness;

    @RequestMapping(method = RequestMethod.GET, value = "/planta")
    public List<Planta> listaPlanta() {
        List<Planta> list = new ArrayList<>();
        try {
            list = plantaBusiness.listaPlanta();
        } catch (Exception e) {
            e.printStackTrace();
            throw new RuntimeException("Error consultando plantas");
        }
        return list;
    }
}
